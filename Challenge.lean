...
/-!
# Public chess-core statement

This is a self-contained statement surface for the Palomar Comparator.  It
keeps the finite board, piece geometry, king-safety predicate, and exhaustive
candidate universe explicit, so the Challenge module does not import the
repository's completed proof development.  The full implementation in
`Chess/` contains the larger orthodox-chess development and its corresponding
generator theorem.
-/
...
/-- Filtering the exhaustive chess move universe is extensionally exact. -/
