...
This Solution exposes the same full production rule kernel as the Challenge
and supplies the proof of `Chess.mateCertificate_rules_adequate`. The theorem
states soundness and completeness of a finite alternating mate-certificate
language for every represented position, attacking colour, bound, and explicit
terminal policy. Positions carry castling rights and en-passant targets; the
move datatype contains normal moves, promotions, en-passant captures, and all
four castling moves. The record type is intentionally generalized and does not
assume `positionInvariant` or reachability.
-/
